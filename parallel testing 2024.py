...
TRL_mapping = {
    'Low: 3 - 4': 1, #H2 Furnace
    'Medium: 6 - 7': 2, #Hybrid Furnace, Electric Furnace
    'High: 8': 3, #Oxyfuel
    'High: 9': 4 #NG
}
...
app.layout = html.Div([
    ...
    html.P([
        html.B([
            "List of Technologies: NG-fired Furnace, NG-Oxyfuel Furnace, Hybrid Furnace (Electric boosting), All Electric Furnace, H",html.Sub("2"),"-fired Furnace"
        ]),
        html.Br(),
...
def update_parallel_coordinates_plot(selected_commodities, cEE_min, cEE_max, cH2_min, cH2_max, cNG_min, cNG_max, cCO2_min, cCO2_max):
    ...
    fig = go.Figure(data=
        go.Parcoords(
            ...
            dimensions=list([
                dict(range=[1, 5], tickvals=[1, 2, 3, 4, 5], ticktext=['NG-fired', 'NG-Oxyfuel', 'Hybrid', 'All-Electric', 'H2-fired'], label='Technology', values=filtered_data['Technology']),
